...
class DSET_processor:
    ...
    def _create_example_cfoee(self, lines):
        # maximum doc length is 543 in train (max input ids 803), 394 in dev, 478 in test
        # too long, so we use a window to cut the sentences.
        W = self.args.window_size
        assert(W%2==0)
        ...
        en2cn_dict = {'subject': '主体', 'object': '客体', 'tool': '工具', 'material': '材料', 'manner': '方式',
                      'source': '事件来源', 'result': '事件结果', 'scope': '事件范围', 'reason': '原因',
                      'background': '背景', 'purpose': '目的', 'time': '时间范围', 'location': '空间范围',
                      'state': '状态', 'degreeAdverb': '程度副词', 'scopeAdverb': '范围副词', 'timeAdverb': '时间副词',
                      'affirmativeAdverb': '肯定副词', 'negativeAdverbs': '否定副词', 'modalAdverb': '情态/语气副词',
                      'placeAdverb': '地点副词'}
        ...
        for line in lines:
            ...
            for sent in line["sentences"]:
                first_word_locs.append(len(text_tmp))
                text_tmp += sent
                sent_dir[sent_id] = sent
                sent_id += 1

            for event_idx, event in enumerate(events):
                event_trigger = dict()
                event_trigger['start'] = event[0]
                event_trigger['end'] = event[1]
                event_trigger['text'] = "".join(full_text[event_trigger['start']:event_trigger['end']])
                event_type='generalEvents'

                offset, min_s, max_e = 0, 0,0
                for i in range(len(sent_dir)):
                    event_trigger['offset'] = offset
                    if i < len(sent_dir)-1:
                ...
                for arg_info in line["gold_evt_links"]:
                    if arg_info[0][0] == event[0] and arg_info[0][1] == event[1]:
                        ...
                        evt_arg = dict()
                        evt_arg['start'] = arg_info[1][0]
                        evt_arg['end'] = arg_info[1][1]
                        evt_arg['text'] = "".join(full_text[evt_arg['start']:evt_arg['end']])
                        if arg_info[2].split('arg', maxsplit=1)[-1] not in en2cn_dict:
                            evt_arg['role'] = arg_info[2].split('arg', maxsplit=1)[-1]
                        else:
                            evt_arg['role'] = en2cn_dict[arg_info[2].split('arg', maxsplit=1)[-1]]
                        if evt_arg['start']<min_s or evt_arg['end']>max_e:
                            self.invalid_arg_num += 1
                        else:
                            evt_arg['start'] -= offset
    ...
    def create_example(self, file_path):# 根据数据集类型调用相应的数据处理方法，如RAMS、ACE-EEQA或WikiEvent
        ...
        elif self.args.dataset_type=='OEE_CFC':
            lines = self._read_jsonlines(file_path)
            return self._create_example_cfoee(lines)
