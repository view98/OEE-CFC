...
class BaseEvaluator:
    ...
    def calculate_one_batch(self, batch):
        ...
        with torch.no_grad():
            loss, outputs_list = self.model(**inputs)
        return outputs_list, named_v,loss


    def evaluate_one_batch(self, batch):
        outputs_list, named_v,loss = self.calculate_one_batch(batch)
    ...
    def evaluate(self):
        ...
        for batch in self.eval_loader:
            ...
            final_loss += loss.detach().cpu().numpy() / self.cfg.logging_steps
        output = self.predict()
        logger.info('smooth_loss: {}'.format(final_loss))
        return output
